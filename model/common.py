...
class TransformerEmbeddings(nn.Module):
    ...
    def forward(self, ids):
        assert len(
            ids.size()) == 2, "Expected input ids to be of size (batches, sequence_len)"
...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim, max_seq_len, n_heads, is_causal=False):
        ...
        if is_causal:  # Will this work?
            self.raw_mask = torch.tril(torch.ones(max_seq_len, max_seq_len)).view(
    ...
    def forward(self, inputs):
        ...
        q = q.view(batches, seq_len, self.n_heads,
                   self.embed_dim // self.n_heads).transpose(1, 2)
        k = k.view(batches, seq_len, self.n_heads,
                   self.embed_dim // self.n_heads).transpose(1, 2)
        v = v.view(batches, seq_len, self.n_heads,
                   self.embed_dim // self.n_heads).transpose(1, 2)

        attn = (q @ k.transpose(-2, -1)) / \
            math.sqrt(self.embed_dim // self.n_heads)

        if self.is_causal:
            attn = attn.masked_fill(
                self.raw_mask[:, :, :seq_len, :seq_len] == 0, float('-inf'))

        attn = F.softmax(attn, dim=-1)

        outputs = (attn @ v).transpose(1,
                                       2).contiguous().view(batches, seq_len, self.embed_dim)
...
class FlashSelfAttention(nn.Module):
    def __init__(self, embed_dim, n_heads, is_causal=False):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        outputs = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, attn_mask=None, dropout_p=0., is_causal=self.is_causal)

        outputs = outputs.transpose(1, 2).contiguous().view(
            batches, seq_len, self.embed_dim)
        outputs = outputs + resid
...
class CrossAttention(nn.Module):
    ...
    def forward(self, inputs, cross_embedings):
        assert inputs.size()[-1] == cross_embedings.size(
        )[-1], "Both embeddings in cross attention must have the same embedding dimension"
